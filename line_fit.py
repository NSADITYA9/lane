...
def final_viz(undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
    ...
    """
    # Generate blank image to draw the lines
    ...
    # Points for left and right lines
    ...
    if len(undist.shape) == 2:
        undist = cv2.cvtColor(undist, cv2.COLOR_GRAY2RGB)
    if len(newwarp.shape) == 2:
        newwarp = cv2.cvtColor(newwarp, cv2.COLOR_GRAY2RGB)
    
    # Resize newwarp if necessary to match the dimensions of undist
    if undist.shape != newwarp.shape:
        newwarp = cv2.resize(newwarp, (undist.shape[1], undist.shape[0]))
...
# Ensure all necessary parameters are defined and passed correctly to the function
